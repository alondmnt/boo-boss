...
const Creatures = (() => {
  ...
  /**
   * Anchor positions for each creature type, used by MonsterTypes to position
   * costume overlays relative to the creature's head and body geometry.
   * - headTop: where a hat/helmet brim sits (top of cranium)
   * - headCenter: centre of face/eyes (for eye overlays)
   * - bodyCenter: centre of torso
   * - scale: overlay size relative to the reference medium creature
   */
  const ANCHORS = {
    ...
    bat:      { headTop: { x: 0, y: -10 }, headCenter: { x: 0, y: -4 },  bodyCenter: { x: 0, y: 0 },  scale: 0.5,  eyes: [{ x: -2.5, y: -4 }, { x: 2.5, y: -4 }] },
  ...
  /** Return anchor positions for a creature type (used by MonsterTypes). */
  function getAnchors(type) {
    return ANCHORS[type] || DEFAULT_ANCHORS;
  }
  ...
  return { create, remove, setPose, getAnchors };
})();
